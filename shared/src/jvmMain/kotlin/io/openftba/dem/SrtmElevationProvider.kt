...
/**
 * Reads SRTM elevation from local `.hgt` / `.hgt.gz` tiles in a folder — no network at
 * runtime. SRTM tiles are 1° squares named after their SW corner (e.g. `N60E029.hgt`):
 * a square grid of big-endian int16 meters, row-major from north to south, west to east.
 * 1201×1201 = 3 arc-second (~90 m), 3601×3601 = 1 arc-second (~30 m). Voids are -32768.
 *
 * Bilinear interpolation between the four surrounding samples; tiles are cached after load.
 */
class SrtmElevationProvider(private val folder: File) : ElevationProvider {
    ...
    private fun loadTile(name: String): Tile? {
        val plain = File(folder, "$name.hgt")
        val gz = File(folder, "$name.hgt.gz")
        val (file, gzipped) = when {
            plain.isFile -> plain to false
            gz.isFile -> gz to true
            else -> return null
        }
        return runCatching {
            val bytes = (if (gzipped) GZIPInputStream(file.inputStream().buffered()) else file.inputStream().buffered())
                .use { it.readBytes() }
